...
results=$(psql -f ./scripts/test-the-release/find-databases.sql)

if [[ "$results" == *"${PGDATABASE}"* ]]; then
  echo "Good news, the database is there."
  exit 0
else
  echo "Something's wrong. The database is not there."
  exit 1
fi
